...
CREATE TABLE IF NOT EXISTS cover_letter_versions (
    ...
    job_description TEXT,                    -- JD used for customization if any
    is_original BOOLEAN DEFAULT false,       -- True for master cover letter versions
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
-- ======================================
-- Add new columns to track cover letter versions
ALTER TABLE applications
...
-- ======================================
-- Update Cascade Deletion Constraints
-- ======================================
...
-- ======================================
-- Data Migration (Optional - for existing cover_letters)
-- ======================================
-- If you have existing cover letters, migrate them to the new structure
-- This creates v1 versions for each existing cover letter
-- Uncomment and run if you have legacy data:

/*
INSERT INTO cover_letters (user_id, title, is_default, created_at, updated_at)
SELECT DISTINCT user_id, title, false, created_at, updated_at
FROM cover_letters_old
WHERE NOT EXISTS (
    SELECT 1 FROM cover_letters cl WHERE cl.user_id = cover_letters_old.user_id
);

INSERT INTO cover_letter_versions (cover_letter_id, version, markdown_content, is_original, created_at)
SELECT cl.id, 'v1', clo.content, true, clo.created_at
FROM cover_letters_old clo
JOIN cover_letters cl ON cl.user_id = clo.user_id AND cl.title = clo.title;

DROP TABLE cover_letters_old;
*/

-- ======================================
-- Utility Functions (Similar to Resume)
-- ======================================
...
BEGIN
    ...
    SELECT 
        ...
        a.position,
        clv.version,
        (a.cover_letter_version_id IS NOT NULL) as has_customized_version
    FROM applications a
...
BEGIN
    ...
    RAISE NOTICE '  - applications (added cover_letter_version_id with RESTRICT cascade)';
    RAISE NOTICE '';
    RAISE NOTICE '✓ Cascade Rules:';
    RAISE NOTICE '  - Original cover letters: RESTRICT (protected if apps reference)';
    RAISE NOTICE '  - Original versions: RESTRICT (protected if apps reference)';
    RAISE NOTICE '  - Customized versions: Can be managed separately';
    RAISE NOTICE '  - Cover letters: Deleted with user (CASCADE)';
    RAISE NOTICE '';
